...
const UCL = {
  ...
  itemClick(event) {
    ...
    if (event.button == 1) {
      ...
      //https://searchfox.org/mozilla-central/rev/7fc94186f1a759790c56e41a90ab350d78dbda30/browser/base/content/utilityOverlay.js#271
  ...
  async create(aLeafName) {
    ...
    if (aLeafName) {
      aLeafName = aLeafName.replace(/\s+/g, " ").replace(/[\\/:*?"<>|]/g, "");
    }
    ...
    IOUtils.exists(this.path).then(value => {
      if (value && isEnable) {
        if (this.sss.sheetRegistered(uri, this.SHEET)) {
          IOUtils.stat(this.path).then(_value => {
            if (this.lastModifiedTime !== _value.lastModified) {
