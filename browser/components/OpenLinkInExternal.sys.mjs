...
async function OpenLinkInExternal(url) {
  ...
  if (platform === "linux") {
    ...
    if (userSelectedBrowserId === "") {
      ...
      const targets = browsers.filter(
        _browser => _browser.keyName === userSelectedBrowserId
      );
      if (targets.length === 0) {
        Services.prompt.asyncAlert(
          null,
      ...
      if (uriWithoutQueryRef == "chrome://browser/content/browser.xhtml") {
        (async () => {
          ...
          openLinkInExternal.addEventListener("command", function (e) {
            const _window = e.currentTarget.ownerGlobal;
            OpenLinkInExternal(
              _window.TabContextMenu.contextTab.linkedBrowser.currentURI.spec
            );
          });
          tabContextMenu.addEventListener("popupshowing", function (e) {
            const _window = e.currentTarget.ownerGlobal;
            const scheme =
              _window?.TabContextMenu?.contextTab?.linkedBrowser?.currentURI
                ?.scheme || "";
        ...
        (async () => {
          ...
          openLinkInExternal.addEventListener("command", function (e) {
            const _window = e.currentTarget.ownerGlobal;
            OpenLinkInExternal(_window.gContextMenu.linkURL);
          });
          contextMenu.addEventListener("popupshowing", function (e) {
            const _window = e.currentTarget.ownerGlobal;
            const scheme = _window?.gContextMenu?.linkURI?.scheme || "";
            e.currentTarget.querySelector(
              "#context-openlinkinexternal"
            ).hidden =
              !(
                _window.gContextMenu.onSaveableLink ||
                _window.gContextMenu.onPlainTextLink
              ) || !/^https?$/.test(scheme);
          });
          contextMenu
